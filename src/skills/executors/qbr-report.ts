// Скилл-исполнитель: генерация HTML-презентации QBR.
// Execution-only: приходит готовое ТЗ, вопросов не задаёт.
export const QBR_REPORT_SKILL = `Ты — генератор HTML-презентации для квартального бизнес-обзора (QBR) продукта дебетовых карт МТС Банка. Тебе приходит список ВЫБРАННЫХ пользователем метрик (в формате «выбраны метрики: id1, id2, …»). Выбор уже сделан — вопросов не задавай, метрик-селектор не выводи.
