...
export default class Bar {
    ...
    draw() {
        this.draw_bar();
        if (!this.gantt.options.hide_progress) {
    ...
    draw_avatar() {
        if (!this.task._avatar_initials || !this.task._avatar_color) return;
        const bx = this.x;
        const bw = this.width;
        const by = this.y;
        ...
        createSVG('circle', {
            ...
            cy: ay,
            r,
            fill: this.task._avatar_color,
            class: 'bar-avatar-circle',
            append_to: this.bar_group,
        });
        ...
        createSVG('text', {
            x: ax,
            y: ay,
            innerHTML: this.task._avatar_initials,
            'text-anchor': 'middle',
            'dominant-baseline': 'central',
            fill: '#fff',
            'font-size': Math.round(r * 0.85),
            'font-weight': '600',
            'font-family': '-apple-system, system-ui',
            'pointer-events': 'none',
            class: 'bar-avatar-text',
            append_to: this.bar_group,
        });
    }

    draw_port() {
        if (this.gantt.options.readonly || this.gantt.options.hide_ports) return;
        const portColor = this.gantt.options.port_color || '#1668dc';
        this.$port = createSVG('circle', {
            cx: this.x + this.width / 2,
            cy: this.y + this.height + 7,
            r: 6,
            fill: '#0e0f14',
            stroke: portColor,
            'stroke-width': '2',
    ...
    draw_resize_handles() {
        ...
        // Skip progress handle when hide_progress is set
        if (!this.gantt.options.readonly_progress && !this.gantt.options.hide_progress) {
    ...
    setup_click_event() {
        ...
        $.on(this.group, 'mouseenter', (e) => {
            timeout = setTimeout(() => {
                ...
                const hl = this.gantt.$container.querySelector(`.highlight-${task_id}`);
                if (hl) hl.classList.remove('hide');
            }, 200);
        });
    ...
    update_bar_position({ x = null, width = null }) {
        ...
        if (x) {
            // Dependency constraint: task must start after ALL parent tasks end
            const xs = this.task.dependencies.map((dep) => {
                const depBar = this.gantt.get_bar(dep);
                return depBar ? depBar.$bar.getEndX() : 0;
            });
    ...
    update_progressbar_position() {
        if (this.invalid || this.gantt.options.readonly || !this.$bar_progress) return;
    ...
    update_port_position() {
        if (!this.$port) return;
        const bar = this.$bar;
        this.$port.setAttribute('cx', bar.getX() + bar.getWidth() / 2);
    }
    ...
    update_handle_position() {
        ...
        if (leftHandle) leftHandle.setAttribute('x', bar.getX());
        if (rightHandle) rightHandle.setAttribute('x', bar.getEndX());
        const handle = this.group.querySelector('.handle.progress');
        handle && handle.setAttribute('cx', this.$bar_progress ? this.$bar_progress.getEndX() : 0);
    }
