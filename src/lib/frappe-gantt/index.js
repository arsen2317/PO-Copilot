...
export default class Gantt {
    ...
    refresh(tasks) {
        ...
        this.setup_tasks(tasks);
        this.change_view_mode();
        // Restore scroll after refresh to prevent jumping
        requestAnimationFrame(() => {
            this.$container.scrollLeft = savedScrollLeft;
        });
    }
    ...
    setup_arrow_interactions() {
        for (let arrow of this.arrows) {
            arrow.setup_delete_button(this.layers.arrow);
        }
    }
    ...
    set_scroll_position(date) {
        ...
        this.$container.scrollLeft = scroll_pos - this.config.column_width / 6;
    ...
    bind_bar_events() {
        ...
        // Port mousedown – start dep-drag
        $.on(this.$svg, 'mousedown', '.port', (e, port) => {
            e.stopPropagation();
            e.preventDefault();
            const bar_wrapper = $.closest('.bar-wrapper', port);
            if (!bar_wrapper) return;
            port_from_id = bar_wrapper.getAttribute('data-id');
            port_cx = +port.getAttribute('cx');
            port_cy = +port.getAttribute('cy');

            $live_path = createSVG('path', {
                fill: 'none',
                stroke: this.options.port_color || '#1668dc',
                'stroke-width': '2',
        ...
        $.on(this.$svg, 'mousedown', '.bar-wrapper, .handle', (e, element) => {
            if (port_from_id) return; // port drag takes priority
        ...
        $.on(this.$container, 'scroll', (e) => {
            ...
            if (x_on_scroll_start > max_end + 100) {
                ...
                this.$adjust.onclick = () => {
                    this.$container.scrollLeft = max_start;
                };
            } else if (
        ...
        $.on(this.$svg, 'mousemove', (e) => {
            // Port drag: draw live bezier path
            if (port_from_id && $live_path) {
                const ox = (e.offsetX || e.layerX);
                const oy = (e.offsetY || e.layerY);
                const d = `M ${port_cx} ${port_cy} C ${port_cx},${port_cy + 40} ${ox},${oy - 40} ${ox},${oy}`;
                $live_path.setAttribute('d', d);
                return;
            }
        ...
        $.on(this.$svg, 'mouseup', (e) => {
            // Port drag end: find target bar and create dep
            if (port_from_id) {
                if ($live_path) {
                    $live_path.remove();
                    $live_path = null;
                }
                const target = e.target ? e.target.closest('[data-id]') : null;
                const to_id = target ? target.getAttribute('data-id') : null;
                if (to_id && to_id !== port_from_id) {
                    this.trigger_event('dep_create', [port_from_id, to_id]);
                }
                port_from_id = null;
                return;
            }
        ...
        document.addEventListener('mouseup', () => {
            if (port_from_id) {
                if ($live_path) { $live_path.remove(); $live_path = null; }
                port_from_id = null;
            }
            is_dragging = false;
    ...
    bind_bar_progress() {
        ...
        $.on(this.$svg, 'mousedown', '.handle.progress', (e, handle) => {
            ...
            if (!$bar_progress) { is_resizing = false; return; }
            $bar = bar.$bar;
