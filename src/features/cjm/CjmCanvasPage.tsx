  ...
  const handleSave = (nodeId: string, data: Partial<CjmNodeData>) => {
    ...
    if (nextArtifactId && nextArtifactId !== prevArtifactId) {
      ...
      setPendingTrigger(
        ...
        `действительно ли он релевантен и актуален для этого пункта CJM. ` +
        `Если да — ОБЯЗАТЕЛЬНО обнови текст пункта через update_cjm (nodeUpdates), кратко (не более 2-3 предложений), ` +
        `сохранив linkedArtifactId. Если нет — объясни почему в чате и предложи отвязать, уточнив, что можешь ` +
        `сделать это сам по команде «отвяжи» (не проси пользователя отвязывать вручную).`,
      );
    }
  };
