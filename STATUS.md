...
### Сессия 9 — 2026-06-29
**Сделано:** системные промпты агентов, кликабельные ссылки, текущая дата, стартовая страница.
- **`AIPanelSider.tsx`**: `SYSTEM_PROMPT` → `getSystemPrompt()` — инжектирует текущую дату (`new Date()`), инструкция «не искать данные старше 3 месяцев». Усилена инструкция по metric chips (`id` в backticks). Добавлена инструкция форматировать внешние источники как `[Название](URL)`.
- **ReactMarkdown**: добавлен кастомный рендерер `<a>` — открывает в новом табе, styled (accent color + underline). Раньше ссылки рендерились без стилей.
- **`data/types/index.ts`**: добавлено поле `systemPrompt?: string` к интерфейсу `Agent`.
- **`data/fixtures/agents.ts`**: добавлен агент «Трендвотчер» (ag4, category: analytics) с полным system prompt (дата + правила чипов + правила ссылок). Все существующие агенты получили `systemPrompt` с инжекцией даты.
- **Роутер**: `index: true` теперь `<Navigate to="/assistant" replace />`. DashboardPage перенесена на `/dashboard`.
- **`AppSidebar`**: ключ дашборда `/` → `/dashboard`, упрощена логика `selectedKey`.
**Решения:** стартовая страница = `/assistant`, дашборд теперь на `/dashboard`. Навигация не изменилась по порядку (ассистент уже был вторым после поиска).
